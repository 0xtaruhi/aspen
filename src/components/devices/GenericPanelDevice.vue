...
const shellClass = computed(() => {
  switch (variant.value) {
    case 'text_lcd':
      return 'h-full w-full overflow-hidden rounded-xl border border-emerald-700/50 bg-[linear-gradient(180deg,rgba(17,24,39,0.96),rgba(10,15,24,0.98))] px-2 py-1.5'
    case 'graphic_lcd':
      return 'h-full w-full overflow-hidden rounded-xl border border-sky-900/60 bg-[linear-gradient(180deg,rgba(8,14,24,0.98),rgba(6,10,18,0.98))] px-2 py-1.5'
    case 'ps2_keyboard':
      return 'h-full w-full overflow-hidden rounded-xl border border-zinc-700 bg-[linear-gradient(180deg,rgba(28,28,33,0.98),rgba(16,16,20,0.98))] px-2 py-1'
    case 'rotary_button':
      return 'h-full w-full overflow-hidden rounded-xl border border-amber-900/50 bg-[linear-gradient(180deg,rgba(30,24,18,0.98),rgba(18,13,10,0.98))] p-2'
    case 'small_keypad':
      return 'h-full w-full overflow-hidden rounded-xl border border-zinc-700 bg-[linear-gradient(180deg,rgba(31,31,36,0.98),rgba(19,19,24,0.98))] px-2 py-1'
    case 'keypad':
    default:
      return 'h-full w-full overflow-hidden rounded-xl border border-zinc-700 bg-[linear-gradient(180deg,rgba(31,31,36,0.98),rgba(19,19,24,0.98))] px-2 py-1'
  }
...
<template>
  <button
    type="button"
    class="h-full w-full text-left transition-transform duration-150"
    :class="interactive ? 'cursor-pointer active:scale-[0.99]' : 'cursor-default'"
    @click="toggle"
  >
    <div :class="[shellClass, 'flex flex-col']">
      <div
        v-if="variant === 'text_lcd'"
        class="flex flex-1 items-center justify-center p-1"
        :class="screenGlowClass"
      >
        <div
          class="rounded-md bg-[radial-gradient(circle_at_top,rgba(74,222,128,0.16),rgba(16,185,129,0.05),transparent_70%)] px-4 py-3 font-mono text-[13px] leading-6 tracking-[0.24em]"
      ...
      >
        <div
          class="grid rounded-md bg-black/35 p-3"
          :class="screenGlowClass"
      ...
      <div v-else class="flex flex-1 items-center p-1">
        <div
          class="grid w-full"
          :style="{
