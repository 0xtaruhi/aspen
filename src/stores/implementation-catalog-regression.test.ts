...
describe('implementation catalog regression', () => {
  beforeEach(async () => {
    projectStore.clearProject()
    projectStore.createNewProject('ImplCatalogProject', 'blinky')
  ...
  it('exposes the current implementation artifact only while the project signature still matches', async () => {
    ...
    expect(implementationCatalogStore.currentImplementationBitstreamPath.value).toBe(
      '/tmp/current.bit',
    )
    expect(implementationCatalogStore.hasStaleImplementationReport.value).toBe(false)
    ...
    expect(implementationCatalogStore.currentImplementationReport.value).toBeNull()
    expect(implementationCatalogStore.currentImplementationBitstreamPath.value).toBe('')
    expect(implementationCatalogStore.hasStaleImplementationReport.value).toBe(true)
  })
})
