...
| [`device-labs/matrix-keypad`](device-labs/matrix-keypad)           | Button, matrix keypad, seven-segment display  | Key scanning and display                     |
...
| Project                                                      | Virtual devices                                                 | What it demonstrates              |
| ------------------------------------------------------------ | --------------------------------------------------------------- | --------------------------------- |
| [`showcases/keypad-calculator`](showcases/keypad-calculator) | Matrix keypad, 4-digit seven-segment display, LED               | A small hexadecimal calculator    |
| [`showcases/mini-synth`](showcases/mini-synth)               | Matrix keypad, rotary encoder, Audio PWM, seven-segment display | A playable monophonic synthesizer |
...
- **Keypad Calculator:** number keys append hexadecimal digits; `A` starts addition, `B` starts
  subtraction, `C` clears, `*` removes one digit, and `#` or `D` evaluates the expression.
- **Mini Synth:** press a keypad key to play a note, rotate the encoder to change octave, and press
  the encoder to return to the default octave.
